...
/// Windows probe backed by netstat2 for the socket table
/// (GetExtendedTcpTable under the hood, which carries the owning pid for
/// every socket) and sysinfo for process metadata.
    ...
    Ok(raw
        ...
        .map(|listener| {
            ...
            ListeningSocket {
                ...
                port: listener.port,
                pid: listener.pid,
                // Windows has no unix uid; when the process join fails
                // (access denied on services) the owner stays honestly
                // unknown, when it works the process user is the owner.
                uid: None,
...
fn raw_listeners() -> Result<Vec<RawListener>, ProbeError> {
    ...
    let infos =
        netstat2::get_sockets_info(families, ProtocolFlags::TCP).map_err(|e| ProbeError::Io {
            path: "GetExtendedTcpTable".to_string(),
...
/// Unlike macOS there is no p_comm truncation to expand and no argv env
/// leak to trim - the name and command arrive as-is (".exe" included;
/// labeling strips that downstream).
...
mod tests {
    ...
    fn probe_sees_own_listener() {
        ...
        let output = WindowsProbe.probe().expect("probe should succeed");
