...
stale processes at once, so port collisions and forgotten servers are annoying to
...
- First target user is Brad: many repos under `~/Code`, frequent port collisions,
...
10. **Shared-port and stale hints** - shared-port detection plus stale dev server hints. Reworked 2026-07-07: bump-conflict inference dropped (bind-time EADDRINUSE fights are unobservable from a snapshot) and the Conflicts tab retired; shared ports surface as a neutral badge and drawer note, and port lookup is the EADDRINUSE debugging path.
11. **Search and filters** - filters for framework, runtime, API, database, Docker, unknown, LAN visible, stale, shared port, plus text search.
...
Ephemeral snapshot plus one small local config file. The `DevSnapshot` is built
per request from live system probing, never persisted, and no telemetry is sent.
Field names below are the working shape; **feature 1 locks the JSON contract**
and every later feature (UI, probes, `--json`) depends on it, so change it in
feature 1 or not at all.
...
- `conflicts` (Conflict[]) - shared-port groups (name is contract-locked; since
  the 2026-07-07 rework the contents are only factual multi-listener ports)
...
- `port` (number) - the shared port
- `service_ids` (string[]) - the listeners sharing it
- `hint` (string) - factual description ("2 processes are listening on :7900 (...)")

### DockerHint

- container/compose identification tied to a service or port

### Local config (feature 13b)

- JSON file in the platform config directory via the `dirs` crate:
  `~/.config/portdoc/config.json` on Linux, Application Support on macOS,
  AppData on Windows.
- Stores ignored services first; later UI preferences and trusted project roots.
...
- **serde / serde_json** - `DevSnapshot` serialization (added at feature 1)
...
- **dirs** - platform config directory for the feature 13b local config
...
- **Dashboard** - default view: summary cards, port lookup, project groups, active services, stale apps, LAN-visible services, Docker hints
...
The Conflicts tab was retired in the 2026-07-07 rework; shared-port facts show
as neutral badges on rows and in the inspect drawer.

API surface: `/api/health`, `/api/snapshot`, `/api/stop`, `/api/reveal`.
Destructive actions (stop/kill) always confirm, graceful first, force only
